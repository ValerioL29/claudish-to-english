...
#   opencode  `opencode run`. Untested here (not installed on the dev machine);
#             flags follow its documented CLI.
#
# Config:
...
$_user"
  ...
  # stdin is closed so no CLI can sit waiting on the hook's (already consumed)
  # input. Backgrounded because stock macOS has no timeout(1): the loop below
  # kills the call on expiry.
  case "$PROVIDER" in
    codex)
      codex exec --sandbox read-only --skip-git-repo-check --ephemeral --color never \
        -C "${TMPDIR:-/tmp}" ${MODEL:+-m "$MODEL"} \
        ${EFFORT:+-c "model_reasoning_effort=${EFFORT}"} \
        -o "$_out" "$_prompt" </dev/null >/dev/null 2>"$_errf" &
      ;;
    agy)
      ...
      ;;
    opencode)
      opencode run ${MODEL:+-m "$MODEL"} "$_prompt" </dev/null >"$_out" 2>"$_errf" &
      ;;
  esac
  _pid=$!
  _t=0
